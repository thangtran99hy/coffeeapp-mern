const {ROLE_ADMIN, ROLE_CASHIER, ROLE_CUSTOMER, ROLE_INVENTORY_MANAGER} = require("../constants");
...
router.get('/user', async (req, res) => {
    try {
        ...
        const user = await User.find({
            role: { $in: [ROLE_ADMIN, ROLE_CASHIER, ROLE_INVENTORY_MANAGER, ROLE_CUSTOMER] },
        })
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip(limit * page);
...
router.patch('/user/:id', getUserById, async (req, res) => {
    ...
    if (emailExist && emailExist._id.toString() !== req.params.id.toString())
        return res.status(400).json({ message: 'Email already exists' });
    if (phoneExist) {
        console.log(phoneExist._id);
        console.log(req.params.id)
    }
    if (phoneExist && phoneExist._id.toString() !== req.params.id.toString())
